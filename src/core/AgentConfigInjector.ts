...
### Sessions (this project only)

Session notes live in \`.grome/sessions/\`. They are briefing documents about what happened in prior sessions **in this same project**. Sessions are NOT synced across projects — use handoffs for that.
...
- \`<YYYY-MM-DD-HHMM>-<slug>.md\` — user-triggered briefing notes written by a prior agent when the user asked them to "write a session" / "leave a note for next agent".
...
**Writing a session note:** when the user says "write a session", "leave a note for the next agent", "session handoff", "write up where we left off", or similar, write a new file to \`.grome/sessions/<YYYY-MM-DD-HHMM>-<slug>.md\` using this format — self-contained, specific, and written for an agent with **zero prior context**:
